...
from models.base_model_bg import BaseModelBG
...
def train(epoch, data_loader, model, log_path, plotter, opts):
    ...
    for iteration in range(1, min(501, len(data_loader))):
            ...
            for add_im in output_image.keys():
                if "Motion" in add_im:
                    for b in range(output_image[add_im].shape[0]):
                        flow = output_image[add_im][b,:2,:,:].permute(2,1,0).cpu().detach().numpy()
                        flow = np.concatenate((flow[:,:,1:2],flow[:,:,0:1]),2)
                        if b == 0 :
                            img8 = flow2img(flow*opts.div_flow)
                            img8 = np.concatenate((img8[:, :, 2:3], img8[:, :, 1:2], img8[:, :, 0:1]), 2)
                            img8 = torch.FloatTensor(img8).permute(2, 1, 0).unsqueeze(0)
                        else:
                            img8_ = flow2img(flow)
                            img8_ = np.concatenate((img8_[:,:,2:3],img8_[:,:,1:2],img8_[:,:,0:1]),2)
                            img8_ = torch.FloatTensor(img8_).permute(2,1,0).unsqueeze(0)
        ...
        if iteration % 100 == 0 :
            """
            Train: Epoch 0: 1/37120 with  
            ssim : 0.0439 |  psnr : -5.2097 |  Perceptual : 1.2399 |  
            Total Loss : 13.2407 |  L1 : 0.8420 |  GAN : -0.0024 |  
            GAN_Feat : 16.5436 |  D_Fake : 1.0024 |  D_real : 0.9974 | 
            """
...
def val(epoch, data_loader, model, log_path, plotter):
    ...
    for iteration in range(1, min(501, len(data_loader))):
        ...
        if (iteration % 100 == 0 ) and ((epoch < 5) or (epoch%10==0)):
            for add_im in output_image.keys():
                if "Motion" in add_im:
                    for b in range(output_image[add_im].shape[0]):
                        flow = output_image[add_im][b,:2,:,:].permute(2,1,0).cpu().detach().numpy()
...
def run(model, Dataset, log_path, plotter, CHECKPOINT_tempfile):
    ...
    for epoch in range(opts.continue_epoch, opts.max_epoch):
        ...
        if loss["Perceptual_bg"] < opts.best_loss_perc:
            checkpoint(
                model, opts.model_epoch_path + "bestperc", CHECKPOINT_tempfile
            )
            #opts.best_epoch = epoch
            opts.best_loss_perc = loss["Perceptual_bg"]
