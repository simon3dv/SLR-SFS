...
if __name__ == "__main__":
    ...
    video_dir = sorted([x for x in os.listdir(pred_dir) if int(x[:5])>=980])
    ...
    for i_video, video in enumerate(video_dir):
        ...
        gtvideo = os.path.join(gt_dir,"%s_gt.mp4"%video)
        flowpath = os.path.join(gt_dir,"%s_motion.pth"%video)
        imgpath = os.path.join(gt_dir,"%s_input.jpg"%video)
