...
if __name__=='__main__':
    ...
    scenes = glob.glob(os.path.join(image_dir,"*.png")) 
    for i_scene, scene_file in enumerate(sorted(scenes)):
        ...
        if end_index != -1 and i_scene > end_index:
            continue
        SCENE = scene_file[-9:-4]
        if ALIGNFILE != "None" and SCENE not in align_dict.keys():
            continue
        images = glob.glob(os.path.join(image_dir,SCENE+"*.png"))
        print("Processing {}".format(images))
        for i_name, image_file in enumerate(sorted(images)): #and '00000' in x])):
            image_name = image_file[-9:-4]
            flow_file = os.path.join(flow_dir,image_name+"_motion.flo")#image_file.replace("input.png", "motion.flo")
            name = image_file[-9:-4]
