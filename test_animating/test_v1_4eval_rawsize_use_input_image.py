...
if __name__=='__main__':
    ...
    with torch.no_grad():
        ...
        for t in tqdm(range(0, FRAME//bs)):
            ...
            with torch.no_grad():
                pred_imgs = model_to_test.model.module.forward_flow(batch,use_input_image=True)
            for key in pred_imgs.keys():
                if "Img" in key:
                    PredImg = F.interpolate(pred_imgs[key], (output_H, output_W), mode='bilinear')
                    PredImg = (PredImg.permute(0,2,3,1).cpu().numpy() * 0.5 + 0.5) * 255
                elif "CompositeFluidAlpha" in key: # Final Alpha
                    PredImg = F.interpolate(pred_imgs[key], (output_H, output_W), mode='bilinear')
                    PredImg = (PredImg.permute(0,2,3,1).cpu().numpy()) * 255
                elif "WarpedAlpha" in key or "RefinedAlpha" in key:
                    PredImg = F.interpolate(pred_imgs[key], (output_H, output_W), mode='bilinear')
                elif "Alpha" in key:
                    PredImg = F.interpolate(pred_imgs[key], (output_H, output_W), mode='bilinear')
                elif "Z_" in key:
                ...
                for tplus in range(0, bs):
                    ...
                    elif ("BG" in key or "Z_" in key or "SmallSpeedAlpha" in key or "FluidAlpha_f" in key):
                        if t==0:
                            save_img = os.path.join(save_dir,"%s.png"%key)
                            if "BGAlpha_f" in key or "FluidAlpha_f" in key or "Z_f" in key:
                                PredImg_t = (PredImg_t - PredImg_t.min()) / (PredImg_t.max() - PredImg_t.min())
                                cv2.imwrite(save_img, PredImg_t.permute(1, 2, 0).cpu().numpy() * 255)
                            else:
                                cv2.imwrite(save_img, (cv2.cvtColor(PredImg_t, cv2.COLOR_RGB2BGR)))
                    elif "WarpedAlpha" in key or "RefinedAlpha" in key:
                        save_img = os.path.join(save_dir, key, "%06d.npy" % (t * bs + tplus))
                        if not os.path.exists(os.path.dirname(save_img)):
                            os.makedirs(os.path.dirname(save_img))
                        np.save(save_img, PredImg_t.cpu().detach().numpy())
                    else:#"CompositeFluidAlpha" in key or "PredImg" in key:
                        save_img = os.path.join(save_dir,key,"%06d.png"%(t*bs+tplus))
                        if not os.path.exists(os.path.dirname(save_img)):
                            os.makedirs(os.path.dirname(save_img))
                        if "CompositeFluidAlpha" in key:
                            cv2.imwrite(save_img, PredImg_t)
                        else:
                            cv2.imwrite(save_img, (cv2.cvtColor(PredImg_t, cv2.COLOR_RGB2BGR)))
