...
def __scale_width(img, target_width, method=Image.BILINEAR):
    ow, oh = img.size
    if (ow == target_width):
        return img
    w = target_width
...
def __colorjitter(img, colorjitter, colorjitter_params):
    if colorjitter:
        ...
        return transforms.ColorJitter(brightness=[brightness,brightness],
                                      ...
                                      hue=[hue,hue])(img)
    return img
def get_transform(opt, size, params, method=Image.BILINEAR, normalize=True):
    transform_list = []
...
class Liquid(data.Dataset):
    def __init__(
        ...
        self.input_transform = Compose(
            [
                Resize((opts.W, opts.W),Image.BILINEAR),
                ToTensor(),
