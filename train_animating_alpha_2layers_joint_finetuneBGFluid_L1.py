...
def train(epoch, data_loader, model, log_path, plotter, opts):
    ...
    for iteration in range(1, min(501, len(data_loader))):
        ...
        if (iteration % 250 == 0 or iteration == 1) and ((epoch<10 and epoch%3==0) or (epoch%20==0)):
            for add_im in output_image.keys():
                ...
                elif "Composite" in add_im or "GTAlpha" in add_im:
                    img8 = output_image[add_im][0:output_image[add_im].shape[0], :, :, :].cpu().data
                ...
                if ((epoch<10 and epoch%3==0)) or (epoch%20==0):
                    plotter.add_image(
...
def run(model, Dataset, log_path, plotter, CHECKPOINT_tempfile):
    ...
    opts.best_epoch = 0
    opts.best_loss = 100000
    opts.best_loss_perc = 100000
    if os.path.exists(opts.model_epoch_path) and opts.resume:
        ...
        opts.best_loss_perc = past_state["opts"].best_loss_perc
    elif opts.resume:
    ...
    for epoch in range(opts.continue_epoch, opts.max_epoch):
        ...
        if opts.RockRegionloss > 0.0:
            opts.RockRegionloss = opts.RockRegionloss - opts.RockRegionloss / opts.RockRegionlossDecay
            opts.FluidRegionloss = opts.FluidRegionloss - opts.FluidRegionloss / opts.RockRegionlossDecay
        ...
        if loss["Perceptual"] < opts.best_loss_perc:
            checkpoint(
                model, opts.model_epoch_path + "bestperc", CHECKPOINT_tempfile
            )
            opts.best_loss_perc = loss["Perceptual"]
...
if __name__ == "__main__":
    ...
    if opts.load_old_model:
        ...
        if opts.load_motion_regressor:
            ...
            for k,v in motion_regressor_model_state_dict.items():
                if "motion_predictor" in k:
                    ...
                    pretrained_dict[k.replace("netD.netD.netD", "netD_motion.netD.netD")] = v
                else:
                    print(k)
            state_dict.update(pretrained_dict)
        if opts.load_bg_model:
            bg_model_state_dict = torch.load(opts.bg_model)["state_dict"]
            pretrained_dict = {
                k: v
                for k, v in bg_model_state_dict.items()
                if ("net_bg" in k)
            }
            state_dict.update(pretrained_dict)
        ...
        for name, param in model.named_parameters():
            #if "model.module.encoder" in name:
            #    param.requires_grad = False
            #if "model.module.projector" in name:
            #    param.requires_grad = False
            #if "model.module.net_bg" in name:
            #    param.requires_grad = False
        """
        print(model)
